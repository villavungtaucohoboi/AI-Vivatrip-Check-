      ...
      <button
        onClick={() =>
          downloadPayslipImage({
            ...
            totalIncome: payslip.total_income,
            totalDeduction: payslip.total_deduction,
            incomeItems: incomeItems.map((i) => ({ name: i.component_name, value: effectiveValue(i) })),
            deductionItems: deductionItems.map((i) => ({ name: i.component_name, value: effectiveValue(i) })),
          })
        }
        className="mb-4 flex w-full items-center justify-center gap-2 rounded-xl border border-teal bg-white py-3 text-[13.5px] font-bold text-teal-dark hover:bg-teal-light"
      >

...
import { RoomTierCards } from "@/components/room-tier-cards";
...
export default async function ProductDetailPage({
  ...
  const isVilla = product.type === "villa";

  const backHref = isVilla ? "/search" : "/search-resort";
  ...
  const pricingContext =
    isVilla && date ? resolveProductPricing(product, { date }, (holidays ?? []) as Holiday[]).context : undefined;

  const heroPrice = pricingContext?.finalPrice ?? pricingContext?.basePrice ?? product.price_weekday;
    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        {activeAmenities.length > 0 && (
          <div className="mt-5">
            <h2 className="mb-2 text-[13px] font-bold tracking-wide text-ink-muted">TIỆN ÍCH CHUNG</h2>
            <div className="grid grid-cols-2 gap-2">
              {activeAmenities.map(({ key, label, Icon }) => (
                <div
                  key={key}
                  className="flex items-center gap-2 rounded-xl border border-border bg-white p-3 text-[12.5px] text-ink"
                >
                  <Icon className="h-4 w-4 shrink-0 text-teal-dark" />
                  {label}
                </div>
              ))}
            </div>
        ...
        <div className="mt-5">
          <h2 className="mb-2 text-[13px] font-bold tracking-wide text-ink-muted">
            CHỌN HẠNG PHÒNG {rates?.length ? `(${rates.length} hạng)` : ""}
          </h2>
          <RoomTierCards
    ...
    <main className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="mt-5 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="text-right">
          <p className="tabular-price font-display text-2xl text-teal-dark">{formatVND(heroPrice)}</p>
          {!date && <p className="text-xs text-ink-muted">giá Thứ 2 - Thứ 5, xem đủ bảng giá bên dưới</p>}
        </div>
      </div>
